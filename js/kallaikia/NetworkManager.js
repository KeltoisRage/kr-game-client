Kallaikia.NetworkManager = function (game) {
	...
	// game.getSocket().send('LoginPrompt', '', '');

	game.getSocket().send('{ host: "localhost", port: 4000, connect: 1, raw: 1, networkManager: 1 }');
	...
	game.getSocket().onOpen('spawn_myself', function (player) {
		spawn(player, true);
		controlling = player.index;
	});

	game.getSocket().onOpen('world state', function (worldState) {
