Kallaikia.Socket = function (o) {
	var self = this, ws = {}, out = o.out || Config.ScrollView, connected = 0;
	var proxy = o.proxy || 'ws://localhost:6200/';
	...
	var onOpen = function(evt) {
		...
		log('Socket: Successfully connected to Kallaikia server!');
	...
	var onClose = function(evt) {
		...
		if (out)
			out.add('<br><span style="color: green;">Kallaikia remote server has disconnected. Refresh page to reconnect!<br></span>');
		...
		log('Socket: Connection to Kallaikia server closed!');
	};
	...
	var onMessage = function (e) {
		...
		if (Config.base64) {
			...
			try {
				...
				log('Socket: Attempt to base64-decode failed:');
				buff += e.data;
			}
		...
		if (!z_lib) {
			try {
				...
				z_lib = 1;
				console.log('Socket: Attempting zlib stream decompression (MCCP).');
			}
		}
